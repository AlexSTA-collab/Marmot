...
/**
 * @file CorrectedInterfaceFiniteElement.h
 * @brief Extended interface finite element formulation for displacement-jump based interface mechanics.
 *
 * This file defines the templated class `Marmot::Elements::CorrectedInterfaceFiniteElement`
...
namespace Marmot::Elements {
  ...
  /**
   * @class CorrectedInterfaceFiniteElement
   ...
   * update (`MarmotCorrectedInterfaceMaterialHypoElastic`) at each quadrature point.
  ...
  class CorrectedInterfaceFiniteElement : public MarmotElement, public MarmotGeometryInterfaceElement< nDim, nNodes > {
  ...
  public:
    ...
    using Material = MarmotCorrectedInterfaceMaterialHypoElastic;
     ...
     */
    CorrectedInterfaceFiniteElement( int                                         elementID,
   ...
   * @brief In-header method definitions for `CorrectedInterfaceFiniteElement`.
   */
  ///@{

  template < int nDim, int nNodes >
  CorrectedInterfaceFiniteElement< nDim, nNodes >::CorrectedInterfaceFiniteElement(
    int                                         elementID,
    FiniteElement::Quadrature::IntegrationTypes integrationType,
  ...
  template < int nDim, int nNodes >
  int CorrectedInterfaceFiniteElement< nDim, nNodes >::getNumberOfRequiredStateVars()
  {
    return qps[0].getNumberOfRequiredStateVars() * qps.size();
  }

  template < int nDim, int nNodes >
  std::vector< std::vector< std::string > > CorrectedInterfaceFiniteElement< nDim, nNodes >::getNodeFields()
  {
    using namespace std;
  ...
  template < int nDim, int nNodes >
  std::vector< int > CorrectedInterfaceFiniteElement< nDim, nNodes >::getDofIndicesPermutationPattern()
  {
  ...
  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::assignStateVars( double* stateVars, int nStateVars )
  {
  ...
  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::assignProperty( const ElementProperties& elementPropertiesInfo )
  {
  ...
  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::assignProperty( const MarmotMaterialSection& section )
  {
    for ( auto& qp : qps ) {
      qp.material = std::make_unique< MarmotCorrectedInterfaceMaterialHypoElastic >( section.materialName,
  ...
  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::assignMaterial( const std::string& materialName,
                                                                        const double*      materialProperties,
                                                                        int                nMaterialProperties )
  {
    for ( auto& qp : qps ) {
      qp.material = std::make_unique< MarmotCorrectedInterfaceMaterialHypoElastic >( materialName,
                                                                                     materialProperties,
                                                                                     nMaterialProperties,
                                                                                     elLabel );
    }
  ...
  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::assignNodeCoordinates( const double* coordinates )
  {
    ParentGeometryElement::assignNodeCoordinates( coordinates );
  }

  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::initializeYourself()
  {
    ...
    for ( QuadraturePoint& qp : qps ) {
      ...
      if ( qp.separationVector.norm() > geometryTolerance && qp.normalSeparation <= geometryTolerance ) {
        throw std::invalid_argument( "CorrectedInterfaceFiniteElement: paired faces have no positive separation in the "
  ...
  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::computeKernels( const double* QTotal_,
                                                                        const double* dQ_,
                                                                        double*       Pe_,
                                                                        double*       Ke_,
                                                                        double        time,
  ...
  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::setInitialConditions( StateTypes state, const double* values )
  {
    switch ( state ) {
    ...
    default:
      throw std::invalid_argument( MakeString() << __PRETTY_FUNCTION__
                                                << ": invalid initial condition for CorrectedInterfaceFiniteElement" );
    }
  }

  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::computeDistributedLoad(
    ...
    double                              dT )
  {
    throw std::invalid_argument( MakeString()
                                 << __PRETTY_FUNCTION__
                                 << ": distributed loads are not implemented for CorrectedInterfaceFiniteElement." );
  }

  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::computeBodyForce( double*       P,
                                                                          double*       K,
                                                                          const double* load,
                                                                          const double* QTotal,
                                                                          double        time,
                                                                          double        dT )
  {
    throw std::invalid_argument(
      MakeString() << __PRETTY_FUNCTION__ << ": body forces are not implemented for CorrectedInterfaceFiniteElement." );
  }

  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::computeConsistentInertia( double* M )
  {
    throw std::runtime_error( MakeString() << __PRETTY_FUNCTION__
                                           << ": inertia is not implemented for CorrectedInterfaceFiniteElement." );
  }

  template < int nDim, int nNodes >
  void CorrectedInterfaceFiniteElement< nDim, nNodes >::computeLumpedInertia( double* M )
  {
    throw std::runtime_error( MakeString() << __PRETTY_FUNCTION__
                                           << ": inertia is not implemented for CorrectedInterfaceFiniteElement." );
  }
  ...
  template < int nDim, int nNodes >
  std::vector< double > CorrectedInterfaceFiniteElement< nDim, nNodes >::getCoordinatesAtCenter()
  {
  ...
  template < int nDim, int nNodes >
  std::vector< std::vector< double > > CorrectedInterfaceFiniteElement< nDim,
  ...
  template < int nDim, int nNodes >
  int CorrectedInterfaceFiniteElement< nDim, nNodes >::getNumberOfQuadraturePoints()
  {
