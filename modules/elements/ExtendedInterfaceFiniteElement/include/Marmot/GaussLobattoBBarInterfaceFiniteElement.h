...
/**
 * @file GaussLobattoBBarInterfaceFiniteElement.h
 * @brief B-bar variant of the Gauss-Lobatto through-thickness interface
 * finite element (GLIQUAD4_BBAR/GLILINE2_BBAR).
 *
 * This file defines the templated class
 * `Marmot::Elements::GaussLobattoBBarInterfaceFiniteElement`, a clone of the
 * validated `GaussLobattoInterfaceFiniteElement` (GLIQUAD4) with ONE
 * structural addition: the pointwise volumetric part of the tangential
 * surface-gradient strain, at every (surface Gauss point, Lobatto station)
 * pair, is replaced by its element-average value over the surface Gauss
 * points -- computed SEPARATELY for every Lobatto station (no averaging
 * between stations). This is the classical B-bar selective-reduced-
 * volumetric-integration technique, applied independently at each
 * through-thickness station.
 *
 * `computeKernels` therefore runs in two passes over the surface
 * quadrature points:
 *
 *  1. First pass (kinematic only, no material calls): for every surface
 *     Gauss point g and station alpha, compute the raw (uncorrected)
 *     trace of A^(alpha)(q_g) = Nminus_alpha A-(q_g) + Nplus_alpha A+(q_g),
 *     and accumulate its J0xW-weighted sum, per station.
 *  2. Compute the per-station element-average trace over the surface
 *     Gauss points.
 *  3. Second pass: for every (g, alpha), build the correction
 *     traceCorrection[alpha] = averageTrace[alpha] - traceRaw[g][alpha],
 *     pass it to `MarmotGaussLobattoBBarInterfaceMaterialHypoElasticN`
 *     (which adds (traceCorrection[alpha]/3) * I to A^(alpha) before the
 *     -- otherwise completely unmodified -- strain reconstruction, local
 *     traction-equilibrium Newton solve, and static condensation), and
 *     assemble Pe/Ke exactly as GLIQUAD4 does.
 *
 * Since the material treats traceCorrection as a fixed external input for
 * its own internal linearization (the standard, classical B-bar
 * "consistent-enough" tangent -- the same corrected operator is used for
 * both the strain and the stiffness, without differentiating the average
 * itself with respect to q), no further changes to Ke's assembly formula
 * are needed: it is identical to GLIQUAD4's.
 */
#pragma once
...
namespace Marmot::Elements {
  ...
  /**
   * @class GaussLobattoBBarInterfaceFiniteElement
  ...
  class GaussLobattoBBarInterfaceFiniteElement : public MarmotElement,
  ...
  public:
    ...
    using Material = MarmotGaussLobattoBBarInterfaceMaterialHypoElasticN< NStations >;
    ...
    GaussLobattoBBarInterfaceFiniteElement( int                                         elementID,
  ...
  GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::GaussLobattoBBarInterfaceFiniteElement(
    int                                         elementID,
    FiniteElement::Quadrature::IntegrationTypes integrationType,
  ...
  template < int nDim, int nNodes, int NStations >
  int GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::getNumberOfRequiredStateVars()
  {
    return qps[0].getNumberOfRequiredStateVars() * qps.size();
  }

  template < int nDim, int nNodes, int NStations >
  std::vector< std::vector< std::string > > GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::
    getNodeFields()
  {
    using namespace std;
  ...
  template < int nDim, int nNodes, int NStations >
  std::vector< int > GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::
  ...
  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::assignStateVars( double* stateVars,
                                                                                           int     nStateVars )
  {
  ...
  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::assignProperty(
  ...
  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::assignMaterial(
  ...
  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::assignNodeCoordinates(
  ...
  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::initializeYourself()
  {
    ...
    for ( QuadraturePoint& qp : qps ) {
      ...
      if ( qp.separationVector.norm() > geometryTolerance && qp.normalSeparation <= geometryTolerance ) {
        throw std::invalid_argument( "GaussLobattoBBarInterfaceFiniteElement: paired faces have no positive separation "
  ...
  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::computeKernels( const double* QTotal_,
                                                                                          const double* dQ_,
                                                                                          double*       Pe_,
                                                                                          double*       Ke_,
                                                                                          double        time,
                                                                                          double        dT )
  {
    ...
    std::array< double, NStations > Nplus, Nminus;
    for ( int alpha = 0; alpha < NStations; ++alpha ) {
      Nplus[alpha]  = 0.5 * ( 1.0 + lobattoXi[alpha] );
      Nminus[alpha] = 0.5 * ( 1.0 - lobattoXi[alpha] );
    }

    // Pass 1 (Section 6): purely kinematic, no material calls. At every
    // surface Gauss point and Lobatto station, compute the raw trace of
    // the interpolated tangential surface gradient A^(alpha)(q_g), and
    // accumulate its J0xW-weighted sum PER STATION (never mixed across
    // stations).
    std::vector< std::array< double, NStations > > traceRaw( qps.size() );
    std::array< double, NStations >                weightedTraceSum;
    weightedTraceSum.fill( 0.0 );
    double weightSum = 0.0;

    for ( size_t qi = 0; qi < qps.size(); ++qi ) {
      const QuadraturePoint& qp    = qps[qi];
      ...
      const Eigen::Matrix< double, nTensor, 1 > APlusFlat  = Bside * dQTop;
      const Eigen::Matrix< double, nTensor, 1 > AMinusFlat = Bside * dQBottom;

      double traceAPlus = 0.0, traceAMinus = 0.0;
      for ( int i = 0; i < nDim; ++i ) {
        traceAPlus += APlusFlat( i * nDim + i );
        traceAMinus += AMinusFlat( i * nDim + i );
      }

      for ( int alpha = 0; alpha < NStations; ++alpha ) {
        const double traceStation = Nminus[alpha] * traceAMinus + Nplus[alpha] * traceAPlus;
        traceRaw[qi][alpha]       = traceStation;
        weightedTraceSum[alpha] += qp.J0xW * traceStation;
      }
      weightSum += qp.J0xW;
    }

    std::array< double, NStations > averageTrace;
    for ( int alpha = 0; alpha < NStations; ++alpha ) {
      averageTrace[alpha] = weightedTraceSum[alpha] / weightSum;
    }

    // Pass 2 (Section 6): the actual constitutive update + assembly,
    // identical to GLIQUAD4's computeKernels except for the additional
    // per-station traceCorrection passed to the material.
    for ( size_t qi = 0; qi < qps.size(); ++qi ) {
      QuadraturePoint& qp = qps[qi];

      std::array< double, NStations > traceCorrection;
      for ( int alpha = 0; alpha < NStations; ++alpha ) {
        traceCorrection[alpha] = averageTrace[alpha] - traceRaw[qi][alpha];
      }
  ...
  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::setInitialConditions( StateTypes    state,
                                                                                                const double* values )
  {
    switch ( state ) {
    ...
    default:
      throw std::invalid_argument( MakeString()
                                   << __PRETTY_FUNCTION__
                                   << ": invalid initial condition for GaussLobattoBBarInterfaceFiniteElement" );
    }
  }

  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::computeDistributedLoad(
    ...
    double                              dT )
  {
    throw std::invalid_argument(
      MakeString() << __PRETTY_FUNCTION__
                   << ": distributed loads are not implemented for GaussLobattoBBarInterfaceFiniteElement." );
  }

  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::computeBodyForce( double*       P,
                                                                                            double*       K,
                                                                                            const double* load,
                                                                                            const double* QTotal,
                                                                                            double        time,
                                                                                            double        dT )
  {
    throw std::invalid_argument( MakeString()
                                 << __PRETTY_FUNCTION__
                                 << ": body forces are not implemented for GaussLobattoBBarInterfaceFiniteElement." );
  }

  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::computeConsistentInertia( double* M )
  {
    throw std::runtime_error( MakeString()
                              << __PRETTY_FUNCTION__
                              << ": inertia is not implemented for GaussLobattoBBarInterfaceFiniteElement." );
  }

  template < int nDim, int nNodes, int NStations >
  void GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::computeLumpedInertia( double* M )
  {
    throw std::runtime_error( MakeString()
                              << __PRETTY_FUNCTION__
                              << ": inertia is not implemented for GaussLobattoBBarInterfaceFiniteElement." );
  ...
  template < int nDim, int nNodes, int NStations >
  std::vector< double > GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::getCoordinatesAtCenter()
  {
  ...
  template < int nDim, int nNodes, int NStations >
  std::vector< std::vector< double > > GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::
  ...
  template < int nDim, int nNodes, int NStations >
  int GaussLobattoBBarInterfaceFiniteElement< nDim, nNodes, NStations >::getNumberOfQuadraturePoints()
  {
